...
////重新测试 回传确认信息
//void send_data_u(void) {
//	 ES=0;//关串口中断
//     TI=0;                                                                                       
//  	 RI=0;                  
//  
//	   SBUF='u';         //Module number of the fluoresence detector (#1/#2) 
//     while(TI==0){}                                   
//     TI=0;

//	   SBUF='u';         //Special bit first 
//     while(TI==0){}                                   
//     TI=0;			 
//			 
//     SBUF='u';        //low 8 bit
...
//B板接收到B开始加热 回传确认信息
void send_data_bback(void) {
	 ES=0;//关串口中断
     TI=0;                                                                                       
  	 RI=0;                  
  
	   SBUF='r';         //Module number of the fluoresence detector (#1/#2) 
     while(TI==0){}                                   
     TI=0;

	   SBUF='r';         //Special bit first 
     while(TI==0){}                                   
     TI=0;			 
...
// Send Temperature reading (mV) to the main controller
{
   ...
   unsigned int temp_rel_L;
     ...
     TI=0;                                                                                       
	   ...
	   SBUF='m';         //Module number of the fluoresence detector (t-#a/m-#b) 
     while(TI==0){}                                   
     TI=0;
...
//B离心回传信息
void send_data_z(void) {
	   ES=0;//关串口中断
     TI=0;                                                                                       
  	 RI=0;                  
  
	   SBUF='z';         //Module number of the fluoresence detector (#1/#2) 
     while(TI==0){}                                   
     TI=0;

	   SBUF='z';         //Special bit first 
     while(TI==0){}                                   
     TI=0;			 
...
void send_data_mixinghold(void) 
   ...
   unsigned int temp_rel_L;
     ...
     TI=0;                                                                                       
	   ...
	   SBUF='f';         //Module number of the fluoresence detector (#1/#2) 
     while(TI==0){}                                   
     TI=0;
...
void send_data_tempdown(void) 
	{
     ...
     TI=0;                                                                                       
	   ...
	   SBUF='h';         //Module number of the fluoresence detector (#1/#2) 
     while(TI==0){}                                   
     TI=0;
...
void send_data_mixingdown(void) 
	{
     ...
     TI=0;                                                                                       
	   ...
	   SBUF='k';         //Module number of the fluoresence detector (#1/#2) 
     while(TI==0){}                                   
     TI=0;
...
//-----------motor_remix  降温混匀----------//		 
void motor_remix_dowm(int cycle,int T)    //remix with heat ;cycle=10;
{
	   int i,k;
	   int tempi=0;
       for (i=1;i<cycle;i++){
...
//	  send_data();
		...
		send_data_tempdown(); //风扇开启，混匀散热 led1灭，led2/led3常亮
//	  delay(200000);//1s
	}
	CON1=1;//stop fan
}
...
//        motor_remix_stay(126,100); //混合伴随加热5min			
	      ...
	      delay(600000);//2s
				fan_en(120);  //风扇开始散热2min 2min后关风扇
				
				send_data_mixingdown();//提前发一句不然屏幕显示会有延迟
  			delay(600000);//0.5s
	      CON1=0;			   //开风扇
				delay(600000);  //2s
//	      motor_remix_dowm(60,20);   //风扇与混合同步进行2min
	      motor_remix_dowm(30,20);
				
	      CON1=1;				//关风扇关混合
				delay(200000);//1s
				delay(200000);	
 				send_data_z();    //发送指令令32开始离心
				con_kinds=5; 			
				break;	
